...
def load_gt_json(myfile, flag):
    ...
    with open(myfile) as json_file:
        for line in json_file.readlines():
            ...
            url = mydict["url"]
            pic_list = str(url).strip().split("/")
            pic_name = pic_list[-1].split(".")[0]
            #pic_name = pic_name.replace("image_","")
            #pic_name = pic_name.replace("img_","")
            for singles in mydict["label"]:
                for single in singles['data']:
                    label = single["class"]
                    bboxes = single["bbox"]
                    bboxes = reduce(lambda x, y: x + y, bboxes)
                    bboxes = map(lambda x: int(x), bboxes)
                    for ss in bboxes:
                        my_str=my_str+str(ss)+","
                    if flag==False:
                        my_str=my_str+label+"\n"
                    else:
                        my_str=my_str[0:-1]+"\n"
            last_result[pic_name] = my_str

    return last_result


def load_res_json(myfile, flag):
    """
    Returns an array with the contents (filtered by fileNameRegExp) of a ZIP file.
    The key's are the names or the file or the capturing group definied in the fileNameRegExp
    allEntries validates that all entries in the ZIP file pass the fileNameRegExp
    ...
    with open(myfile) as json_file:
        for line in json_file.readlines():
            ...
            #pic_name = pic_name.replace("image_","")
            #pic_name = pic_name.replace("img_","")
            for single in mydict["texts"]:
                for ss in single["bboxes"]:
                    my_str=my_str+str(ss)+","
                if flag==False:
                    my_str=my_str+"abc"+"\n"
                else:
